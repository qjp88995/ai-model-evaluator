...
import {
  ...
  Space, Tag, Popconfirm, message, Tooltip,
} from 'antd';
...
export default function ModelsPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    form.resetFields();
    form.setFieldsValue({ temperature: 0.7, topP: 1.0, maxTokens: 2048, timeout: 30000, retryCount: 2, isJudge: false, isActive: true });
    setModalOpen(true);
  };
  ...
  const providerColors: Record<string, string> = {
    openai: 'green', anthropic: 'orange', zhipu: 'blue',
    moonshot: 'purple', qianwen: 'cyan', custom: 'default',
  };
  ...
  const columns = [
    ...
    {
      title: '裁判模型', dataIndex: 'isJudge', key: 'isJudge', width: 90,
    ...
    <>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="apiKey" label={editing ? 'API Key（留空保持不变）' : 'API Key'} rules={editing ? [] : [{ required: true }]}>
